...
export const getAllAddressesHandler = function (
  this: any,
  schema: any,
  request: any
) {
...
export const addNewAddressHandler = function (
  this: any,
  schema: any,
  request: any
) {
...
export const editAddressHandler = function (
  this: any,
  schema: any,
  request: any
) {
  ...
  try {
    ...
    userAddresses = userAddresses.map((item: any) =>
...
export const removeAddressHandler = function (
  this: any,
  schema: any,
  request: any
) {
  ...
  try {
    ...
    let userAddresses = schema.users.findBy({ _id: userId }).address;
    const addressId = request.params.addressId;
    userAddresses = userAddresses.filter((item: any) => item._id !== addressId);
    this.db.users.update({ _id: userId }, { address: userAddresses });
    return new Response(200, {}, { address: userAddresses });
  } catch (error) {
    return new Response(
